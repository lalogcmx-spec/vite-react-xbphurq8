...
- **OCR de tickets**: `extractTicketData()` en `app.ts` usa `openai.beta.chat.completions.parse()` con `gpt-4o-mini` + Structured Outputs (Zod) para extraer comercio/folio/total/fecha de la imagen del ticket. Lógica correcta, nunca probada contra una imagen real porque requiere `OPENAI_API_KEY` real (el `.env` actual tiene placeholders).
...
- **Vision Engine compartido** (`server/src/drivers/visionEngine.ts`, 318 líneas): loop de automatización con Playwright + fallback a GPT-4o Vision cuando un selector CSS falla. Nunca ejecutado contra un sitio real en este entorno (sandbox sin acceso a internet general).
...
- **Drivers genéricos Walmart/Starbucks/Zara** (`server/src/drivers/genericVisionDriver.ts`): 100% dependientes de GPT Vision, se niegan a arrancar si no defines `MERCHANT_PORTAL_WALMART/STARBUCKS/ZARA` en `.env` (a propósito, para no inventar URLs).
...
- **Los selectores CSS de OXXO/Costco son no verificados.** Si el formulario real usa nombres distintos a los asumidos, el "fast path" fallará y todo dependerá del fallback de GPT Vision (que sí tiene mejor probabilidad de funcionar, pero tampoco se ha probado un solo intento real).
...
2. Conseguir credenciales reales: OpenAI API key, Meta WhatsApp Cloud API (token + phone_number_id + verify token), SMTP.
...
```
APP_PORT, SUPABASE_URL, SUPABASE_SERVICE_KEY, OPENAI_API_KEY, REDIS_URL,
